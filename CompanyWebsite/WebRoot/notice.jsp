...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/bootstrap-admin-theme.css">
      ...
      <script src="${pageContext.request.contextPath}/js/noticeInfo.js"></script>
</head>
    ...
    <div class="container-fluid"> 
        <div class="navbar-header"> 
          ...
          <ul class="nav navbar-nav">
            ...
            <li class="active"><a href="${pageContext.request.contextPath}/noticeAction_findNoticeByPage.action">企业公告</a></li>
			...
			<div class="col-lg-12">
                            ...
                            <s:iterator value="#request.pb.beanList" var="notice">
                             <tbody>
	                                ...
	                                <td><s:date name="#notice.ntime" format="yyyy-MM-dd" /></td>   
	                                 <td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#findModal" onclick="getNotice(<s:property value="#notice.nid"/>)" >查看</button></td>                                          
                          	  </tbody>
                            </s:iterator>
                            </s:if>
                            <s:else>
				...
				<s:if test="#request.pb!=null">
                        ...
                        <div class="pull-right"><!--右对齐--->
                           ...
                           <li><a href="${pageContext.request.contextPath}/noticeAction_${pb.url }pageCode=1">首页</a></li>
                           <li><a href="${pageContext.request.contextPath}/noticeAction_${pb.url }pageCode=${pb.pageCode-1 }">&laquo;</a></li><!-- 上一页 -->
                           <%-- 循环显示页码列表 --%>
								<c:forEach begin="${begin }" end="${end }" var="i">
								  ...
								  <%--如果是当前页则设置无法点击超链接 --%>
								  	...
								  	<c:otherwise>
								  		<li><a href="${pageContext.request.contextPath}/noticeAction_${pb.url }pageCode=${i}">${i}</a></li>
								  	</c:otherwise>
								  </c:choose>
								</c:forEach>
				        	   <%--如果当前页数没到总页数，即没到最后一页,则需要显示下一页 --%>
							  <c:if test="${pb.pageCode < pb.totaPage }">
								  <li><a href="${pageContext.request.contextPath}/noticeAction_${pb.url }pageCode=${pb.pageCode+1}">&raquo;</a></li>
							</c:if>
							<%--否则显示尾页 --%>
							<li><a href="${pageContext.request.contextPath}/noticeAction_${pb.url }pageCode=${pb.totaPage}">尾页</a></li>
							</ul>
                           </div>
                    </s:if>           
